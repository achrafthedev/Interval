  ...
  return (
    <div className="flex flex-col items-center min-h-full px-4 pb-24 md:pb-8 pt-8 md:pt-12">
      <div className="w-full max-w-lg">
        <h1 className={`text-2xl font-bold ${textPrimary} mb-1`}>Date Calculator</h1>
        <p className={`text-sm ${textMuted} mb-8`}>Calculate days between dates or add/subtract days from a date.</p>
        ...
        <div className={`flex rounded-xl border ${border} p-1 mb-6`}>
          {(['between', 'add'] as const).map((m) => (
            <button key={m} onClick={() => setMode(m)} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${mode === m ? 'bg-indigo-600 text-white' : textSecondary}`}>
              {m === 'between' ? 'Days Between' : 'Add / Subtract'}
            </button>
          ))}
        </div>
        ...
        {mode === 'between' ? (
          <>
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div><label className={`text-xs ${textMuted} block mb-1.5`}>Start Date</label><input type="date" value={dateA} onChange={(e) => setDateA(e.target.value)} className={ic} /></div>
              <div><label className={`text-xs ${textMuted} block mb-1.5`}>End Date</label><input type="date" value={dateB} onChange={(e) => setDateB(e.target.value)} className={ic} /></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {[{ v: between.days, l: 'Days' }, { v: between.weeks, l: 'Weeks' }, { v: between.months, l: 'Months' }, { v: between.hours.toLocaleString(), l: 'Hours' }, { v: between.minutes.toLocaleString(), l: 'Minutes' }, { v: (between.days * 24 * 3600).toLocaleString(), l: 'Seconds' }].map(({ v, l }) => (
          ...
          <>
            <div className="space-y-3 mb-6">
              ...
              <div className="flex gap-3">
                <div className="flex-1"><label className={`text-xs ${textMuted} block mb-1.5`}>Days</label><input type="number" min={0} value={addDays} onChange={(e) => setAddDays(parseInt(e.target.value) || 0)} className={ic} /></div>
                <div><label className={`text-xs ${textMuted} block mb-1.5`}>Direction</label>
                  <select value={addDir} onChange={(e) => setAddDir(e.target.value as any)} className={ic}><option value="add">Add (+)</option><option value="subtract">Subtract (−)</option></select>
                </div>
              </div>
            </div>
            <div className={`p-6 rounded-2xl border ${border} ${surface} text-center`}>
              <div className={`text-xs ${textMuted} mb-2`}>Result</div>
